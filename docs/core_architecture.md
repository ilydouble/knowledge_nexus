...
- **物理资产线**：负责文件、目录、对象存储、预览、下载、审计和硬权限。由网盘（Cloudreve 或其他）负责，Nexus 不介入。
- **语义知识线**：负责实体、标签、图谱、双向链接和 GraphRAG 上下文。由 Nexus 作为旁路层负责，不改动文件本身。
...
物理文件和知识图谱是两个完全独立的系统，通过文件 URI 松耦合：

```
Cloudreve（或任意网盘）          Neo4j 知识图谱
  原始文件 ───────────────────→  File 节点（uri）
  目录结构                         ↓ 提取
  权限管理                       Entity 节点 + 关系边
  全文检索（原生）                 摘要、标签（Postgres）
```

**实际效果**：换网盘只需实现 3 个方法（`list_files` / `get_file_content` / `iter_file_events`），图谱、Pipeline、MCP Server 全部不动。见 [storage-adapter-integration.md](./storage-adapter-integration.md)。

---

### 2.2 成本意识：在 LLM 之前做最大化过滤

每一步都设计为"能早退就早退"，将 LLM API 调用量压到最低：

```
文件进入
  │
  ▼
FileGate ──× 图片/视频/压缩包 → 直接跳过，零消耗
  │
  ▼
DocumentClassifier
  │
  ├── tabular_data (Excel) → 只读表头+行数+3行样本 → 1次LLM调用
  │
  └── 其他文档
        │
        ├── ≤10,000字 → 1次LLM调用
        │
        └── >10,000字 → Map-Reduce（分段并行，不靠扩大上下文）
```

**向量嵌入也遵循同一原则**：默认 `VECTOR_BACKEND=none`，Cloudreve 原生全文检索已经够用，只有需要语义问答时才开启。

---

### 2.3 图谱越用越好：Experience Replay 闭环

当前的图谱是**文档级静态提取**，细粒度知识（某段具体推理、某个边缘案例）不强行预提取，留给 Agent 运行时按需探索：

```
现在（已实现）
  文档 → LLM提取 → 图谱（结构化知识，成本可控）
                ↑
           MCP Server（Claude Code 可查询）

未来（Experience Replay）
  Agent 运行时探索 → 发现细粒度知识
                        ↓
                   回写 Neo4j 图谱
                        ↓
...
这个设计的关键洞察：**被实际使用过的知识比盲目预提取的知识质量更高**，因为它带有使用场景和验证历史。MCP Server 是这个闭环的基础接口。
