...
- 不再把外部系统作为目标依赖；当前方向是基于 Pi-Agent 和现有 Knowledge OS 代码独立抽取实体关系、审核候选、写入 Neo4j 并进行图谱问答。
...
- `smart_campus` 模板已补充 Pi-Agent 原生构图治理概念：`GraphWorkspace` 和 `CandidateBatch`。
...
| 去掉外部交付系统依赖 | Pi-Agent 可以直接通过现有候选抽取、审核、Neo4j 入库和图谱问答能力形成闭环 |

## 遇到的问题
| 问题 | 解决方案 |
|------|---------|
| 第一版 Pi-Agent 技能优先级尚未明确 | 先给出原生构图闭环，再让用户确认优先实现本体初始化、候选审核或故障诊断 |
